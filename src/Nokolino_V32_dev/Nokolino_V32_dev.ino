/* Nokolino V3.2 23.01.2021 - Nikolai Radke
 *  
 ...
 *  DFPlayer: Works with JL-AA19HFF851-94, MH2024K-24SS, YX5200-24SS 
 *            GD3200B makes too much noise
 *  
 *  Flash-Usage: 3.650 (IDE 1.8.19 | AVR 1.8.3 | ATtiny 1.0.2 | Linux X86_64 | ATtiny85 )
 *  
 *  Circuit:
 *  1: RST | PB5  free
...
#define minCurrent   3.30 +Offset*1000 // Low power warning current + measuring error
#define battLow      3.10 +Offset*1000 // Minimal voltage before DFPlayer fails
...
char     count_files;                 // Files in directory
...
while(1) {
  ...
  // Optional: Check current
    ...
    if (counter==0) {
      current=MeasureVCC();
      if (current<=minCurrent) {          // Current below minimum
        if (current<=battLow) low=true;   // Power too low
...
uint16_t MeasureVCC(void) {           // Thank you, Tim!
    ...
    newdelay(5);                      // wait for Vref to settle
    ...
    low = ADCL;
    adcVal = (ADCH << 8) | low;
    return ((long)1024 * 1100) / adcVal * 1000;
}
