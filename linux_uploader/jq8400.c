/*
 * jq8400 - Program the SPI flash of a JQ8400 MP3 player module.
 *
...
usage(void)
{
    printf("Usage: jq8400 <command> [OPTION]... [FILE]...\n"
	   "\n"
	   "Write files to an MP3 player based on the JQ8400 chip that is connected\n"
	   ...
	   "\n"
	   "Commands:\n"
	   "  -L          List available JQ8400 devices\n"
...
jq_identify(char *devname)
{
    ...
    ret = (strcmp(vendor, "JQ8400  ") == 0 &&
	   strcmp(product, "uart mp3 player ") == 0);

 error1:
    close(dev);
    
 error2: 
    p(2, "found %s JQ8400 module on %s\n", ret ? "a" : "no", devname);
    p(3, "\n");
...
jq_find(const char *pattern)
{
    ...
    if (devname == NULL) {
	errx(1, "cannot find a JQ8400 module.");
    }
    if (verbosity < 2) {
	p(1, "found a JQ8400 module on %s\n", devname);
    } else {
	p(1, "using %s\n", devname);
    }
    return devname;
...
patch_iso(uint8_t *buf, int size, int flashsize)
{
    ...
    *isosize = htole32(size);
    ...
    char volname[33];
    int len = snprintf(volname, 33, "JQ8400 %dM", flashsize / (1024*1024));
...
main(int argc, char **argv)
{
    ...
    if (device) {
	if (force) {
	    p(0, "using %s as a JQ8400 module without check.\n", device);
	} else if (!jq_identify(device)) {
	    errx(1, "%s does not look like a JQ8400 module, add -f to use it anyway.", device);
	}
    } else {
